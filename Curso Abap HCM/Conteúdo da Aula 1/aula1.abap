*Transações de HCM do submódulo PA abordadas na aula::
"PA20 - Consulta de funcionários.
"PA30 - Permite tomar ações encima de cada infotipo.

*Infotipos:
"Oque é um infotipo: é uma estrutura que armazena os dados de um funcionário dentro do banco de dados. O infotipo é um código numérico, normalmente de quatro dígitos. Existem submodulos de infotipos, na 
"aula 1, formam apresentados apenas infotipos do submódulo PA, os infotipos deste módulo sempre começam em 0000 e vão até 0999.

*Principais ifotipos do submódulo PA abordados na aula::
"0000 - Infotipo de ações para um funcionário.
"0001 - Infotipo que faz a ligação entre funcionário e empresa. 
"0002 - Infotipo de informações pessoais do funcionário.
"0006 - Infotipo de endereços do funcionário.
"0007 - Infotipo de quantidade horas que o funcionário trabalha.
"21 - Infotipo de cadastro de membros da família(dependentes).

*Como saber em qual tabela estão os dados de um infotipo: basta juntar as síglas do submódulo (neste caso é o PA) + o número do infotipo.
"Ex: quero saber o nome da tabela onde estão os dados do infotipo 0021. PA + 0021 = PA0021 (nome da tabela).


